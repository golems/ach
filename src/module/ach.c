...
rdlock(ach_channel_t* chan, int wait, const struct timespec* reltime)
{
...

  {
	...
	res = wait_event_interruptible_timeout(shm->sync.readq,
					       ((chan->seq_num != shm->last_seq)|| chan->cancel),
	...
	mutex_unlock(&shm->sync.mutex);
	if (wait_event_interruptible(shm->sync.readq, ((chan->seq_num != shm->last_seq)|| chan->cancel))) {
	  return ACH_BUG;
	}
	if (chan->cancel) {
	  return ACH_CANCELED;
...
static struct ach_header*
ach_create(size_t frame_cnt, size_t frame_size)
{
	 ...
	 ach_put_fun transfer, void *cx, const void* obj, size_t len)
{
  ...
  else { ACH_XPUT_ASSERT(0 == index_ar[shm->index_head].seq_num);}
	 ...
	 size_t* frame_size)
{
  ...
  if( (chan->seq_num == shm->last_seq && !o_copy) || 0 == shm->last_seq) {
	...
	if (chan->seq_num == shm->last_seq) {
      ...
      /* read from the index */
      retval = ach_xget_from_offset( chan, read_index, transfer, cx, pobj, frame_size);
  }
...
static enum ach_status
ach_cancel(ach_channel_t* chan, unsigned int unsafe)
{
  if (!unsafe) {
    enum ach_status r = wrlock(chan);
    if (ACH_OK != r) {
      printk(KERN_WARNING "Read lock failed => Unsafe cancel\n");
      unsafe = 1;
    }
  }
...
static long ach_ch_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
  ...
  case ACH_CH_SET_MODE: {
    ...
    if (ch_file->mode.reltime.tv_sec != 0 || ch_file->mode.reltime.tv_nsec != 0)
      KDEBUG2("Setting wait time to %ld.%09ld\n", 
	     ch_file->mode.reltime.tv_sec, ch_file->mode.reltime.tv_nsec);
  ...
  case ACH_CH_GET_STATUS: {
...

    {
      ...
      uint64_t oldest_seq = index_ar[oldest_index].seq_num;
      ...
      if (oldest_seq > ch_file->seq_num) {
        ...
        stat.new = shm->last_seq - ch_file->seq_num;
      }
      ...
      if (-EFAULT == copy_to_user((void*)arg, &stat, sizeof(stat))) {
        ret = -EFAULT;
      }
    }
    break;
...
static ssize_t ach_ch_write(struct file *file, const char* buffer, size_t len, loff_t *offset)
{
  struct ach_ch_file* ch_file = (struct ach_ch_file*)file->private_data;
...
static ssize_t ach_ch_read(struct file *file, char* buffer, size_t len, loff_t *offset)
{
...
static int ach_ch_setup_cdev(struct ach_ch_device* dev)
{
  ...
  if (ret < 0) {
    printk (KERN_ERR "ach: Adding device to kernel failed (minor=%u)\n",dev->minor);
      return ret;
  }
...
static int ach_ch_remove_cdev(struct ach_ch_device* dev)
{
  device_destroy(ctrl_data.ach_ch_class, MKDEV(ctrl_data.major, dev->minor));
  cdev_del(&dev->cdev);
 ...
 **********************************************************************************/
static long ach_ctrl_ioctl(struct file* file, unsigned int cmd, unsigned long arg)
{
  int ret = 0;
  ...
  case ACH_CTRL_CREATE_CH: {
    struct ach_ctrl_create_ch* create_arg = (struct ach_ctrl_create_ch*)arg;
...

    {
      ...
      ret = ach_ch_device_init(dev, create_arg->frame_cnt, create_arg->frame_size);
      if (ret) {
  ...
  case ACH_CTRL_UNLINK_CH: {
    struct ach_ctrl_unlink_ch* unlink_arg = (struct ach_ctrl_unlink_ch*)arg;
...
static int __init ach_init(void)
{
  ...
  /* We'll use own major number as we want to control the minor numbers ourself */
  {
    ...
    if (ret < 0) {
      printk(KERN_ERR "ach: Failed to allocate major number with %u minor numbers\n", max_devices);
